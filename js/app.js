...
let sumTwo = sum (sumOne,c)[0];  //sumTwo equal the sum of a+b+c
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for (var i = 0; i < sumArr.length; i++){
    totalRun = sum (totalRun, sumArr[i])[0];
  }
    let message = `${testArray} was passed in as an array of numbers, and ${totalRun} is their sum.`;

  return [totalRun, message];
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
 let dynamicTest = 1;
  ...
  dynamicTest = multiply(dynamicTest, dynamicArray[i])[0]; 
  
 }
 let message = `The numbers ${dynamicArray} have a product of ${dynamicTest}.`
